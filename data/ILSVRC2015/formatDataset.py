...
pathToDataset = "..\\code-master\\dataset\\twitch\\v1\\v1"
indexNumIm = -5
numNumbers = 4
...
for folder in folders[:-1]:
    ...
    for image in images:
        ...
        if(indexIm == 0) or (previousImageName != "" and (int(previousImageName[indexNumIm]) + 1) % 10 != int(image[indexNumIm])):

            # make new folder for video data
            currVidData = path.join("Data\\VID" , f"{numVids:04d}")
            pathlib.Path(currVidData).mkdir(parents=True, exist_ok=True)

            # make new folder for annotations
            currVidAnnotations = path.join("Annotations\\VID", f"{numVids:04d}")
        ...
        newImName = f"{indexIm * periodInFrames:06d}" + ".JPEG"
        os.rename(path.join(currVidData, image), path.join(currVidData, newImName))

        annotationName = image[:indexNumIm + 1] + ".xml"
        shutil.copyfile(path.join(pathToDataset, folder, "image_annotation", annotationName), path.join(currVidAnnotations, annotationName))          
        newAnnotationName = f"{indexIm * periodInFrames:06d}" + ".xml"
        os.rename(path.join(currVidAnnotations, annotationName), path.join(currVidAnnotations, newAnnotationName))

        if(numVids < 2000):
            f.write(f"{numVids-1:04d}" + " 1 " + str(indexIm * periodInFrames) + " " + str(165) + '\n')
        else:
            t.write(f"{numVids-1:04d}" + " 1 " + str(indexIm * periodInFrames) + " " + str(165) + '\n')
